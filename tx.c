...
//      printk(KERN_DEBUG "in add tx raidotap %u \n",seq);
	...
	//memcpy(skb->cb,(char *)seq,4);
	//memcpy(skb->cb+4,(char *)ack,4);
	//memcpy(skb->cb+4,(char *)totlen,2);
        //printk(KERN_DEBUG "hello peichanghua %s in tx.c skb cb\n",skb->cb);
	struct timespec ts;
        getnstimeofday(&ts);
	...
	printk(KERN_DEBUG "hello in tx.c tstamp tv=%d \n",skb->tstamp.tv.sec);
	   ...
	   // printk(KERN_DEBUG "%ld,%ld,%u,%u,%d\n",ts.tv_sec,ts.tv_nsec,ip_id,ip_off,totlen); //comment by sigcomm
